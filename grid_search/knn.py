...
def main():
 ...
 for k in list_k:
  ...
  for i in range(1,6):
    ...
    ses.append(knn['sensibilidade'])
    esp.append(knn['especificidade'])
    f1.append(knn['f1'])
    # Salva os resultados
  with open('knn.csv', 'a+') as file:
    file.write(f"{k};{i};{sum(ses)/len(ses)};{sum(esp)/len(esp)};{sum(f1)/len(f1)}\n")
